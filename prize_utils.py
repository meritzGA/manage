...
def detect_prize_structure(columns, labels=None):
    ...
    if "브릿지시상금" in cols:
        ...
        bridge = {
            ...
            # 다음(형성중) 브릿지(브릿지실적_{cm}_N월)가 있으면 현재 단일월 브릿지는 이미 확정됨
            "settled":   bool(cm) and any(
                re.match(rf"^브릿지실적_{cm}_\d+월$", c) for c in cols
            ),
        }
...
def calculate_prize_for_code(target_code, prize_config, df_src):
    ...
    if ps.get("bridge"):
        ...
        if b.get("settled"):   # 확정된 브릿지: "목표까지 X원 부족" 독려 문구 제거
            sf, tgt = 0, 0
