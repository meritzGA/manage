    ...
    if submit_login and manager_code:
        ...
        if my_df.empty:
            ...
            try:
                ...
                # 최종 순서
                # 비활성 주차 컬럼 방어적 제거 (layout의 active_weeks 기준)
                _layout = st.session_state.get('layout') or {}
                _active_weeks = set(_layout.get('active_weeks', []))
                if _active_weeks:
                    def _is_inactive_week(colname):
                        mm = re.search(r'(\d+)\s*주', str(colname))
                        return mm is not None and int(mm.group(1)) not in _active_weeks
                    display_cols = [c for c in display_cols if not _is_inactive_week(c)]
